...
process.stdin.on("end", function() {
  ...
  Promise.resolve().then(() => (init_src(), src_exports)).then(({ default: fetch2, Headers: Headers2, Request: Request2 }) => {
    init.body = init.body ? Buffer.from(init.body, "base64") : void 0;
    init.headers = new Headers2(init.headers);
    if (init.timeout) {
    ...
    return fetch2(new Request2(resource, init), {});
  }).then(
    (response) => response.arrayBuffer().then((buffer) => respond(serializeResponse(Buffer.from(buffer).toString("base64"), response))).catch((error) => {
